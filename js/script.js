...
document.addEventListener('DOMContentLoaded', function() {
    // Ensure email is displayed correctly (fixes obfuscation issues)
    const emailLinks = document.querySelectorAll('a[href^="mailto:"]');
    emailLinks.forEach(link => {
        const emailAddress = link.getAttribute('href').replace('mailto:', '');
        // If the displayed text is obfuscated or empty, replace it with actual email
        if (link.textContent.includes('[email') || link.textContent.includes('protected') || link.textContent.trim() === '') {
